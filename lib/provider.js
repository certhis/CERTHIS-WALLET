module.exports = function (certhisWallet, request, addFundPopup, disconnect) {
  var storageC = require("./storageAdaptater");
  const { Web3 } = require("web3");
  const EthereumUtil = require("ethereumjs-util");

  return {
    async getProvider(rpc, wallet_address) {
      ...
      const cryptoPopup = require("./cryptoPopup")(certhisWallet, disconnect);
      ...
      provider.send = async function (args, callback) {
        ...
        if (method == "eth_accounts") {
          ...
          var last_hash =
            storageC.localStorageAdaptater.getItem("paytweedHash");
          if (last_hash != undefined && last_hash != "") {
            storageC.localStorageAdaptater.setItem("paytweedHash", "");
          ...
          //check  force_sign
          var force_sign = storageC.localStorageAdaptater.getItem("force_sign");
          var validPopup = false;
          if (force_sign == "true" || force_sign == true) {
            validPopup = true;
          } else {
            validPopup = await popupLib.showSignPopup(
