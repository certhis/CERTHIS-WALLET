      ...
      if (get_tx != null) {
        ...
        if (get_tx.value != undefined) {
          InsertHTMLBOX =
            ...
            web3.utils.fromWei(String(Number(get_tx.value))) + ' '+rpcinfo.currency_array[localStorage.removeItem("RPC_ID_CERTHIS")]+
            "</div></div>";
        } else {
          InsertHTMLBOX = InsertHTMLBOX + '<div style="    border-bottom: 2px solid #ccc;padding-bottom: 10px;margin-bottom: 15px;"> '+
          '<div style="display: inline-block;width: 50%;text-align: left;padding-left: 15px;font-size: 20px;">Value</div>'+
          '<div style="display: inline-block;width: 50%;text-align: right;padding-right: 15px;font-size: 20px;">0 '+rpcinfo.currency_array[localStorage.removeItem("RPC_ID_CERTHIS")]+'</div>'+
          '</div>';
        }
        ...
        InsertHTMLBOX =
          ...
          '<div style="display: inline-block;width: 50%;text-align: right;padding-right: 15px;">' +
          web3.utils.fromWei(String(Number(get_tx.gas))) +' '+rpcinfo.currency_array[localStorage.removeItem("RPC_ID_CERTHIS")]+
          "</div></div>" +
          "</div>";
