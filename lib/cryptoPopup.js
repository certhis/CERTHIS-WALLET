module.exports = function (certhisWallet, disconnect) {
  ...
  function generateSecretKey() {
    try {
      ...
      console.log("generateSecretKey", e);
      return "";
    }
  }

  // Function to get or create a session secret key
  function getSessionSecretKey() {
    try {
      let sessionKey = storageC.localStorageAdaptater.getItem("SESSION_SECRET_KEY");
      if (!sessionKey) {
      sessionKey = generateSecretKey();
      storageC.localStorageAdaptater.setItem("SESSION_SECRET_KEY", sessionKey);
    }
      return sessionKey;
    } catch (e) {
      console.log("getSessionSecretKey", e);
      return "";
    }
  }
    ...
    getDecryptedPrivate_key: async function () {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (
            storageC.localStorageAdaptater.getItem("store_key") == "true" ||
            storageC.localStorageAdaptater.getItem("store_key") == true
          ) {
            if (
              storageC.localStorageAdaptater.getItem("stored_key") != "" &&
              storageC.localStorageAdaptater.getItem("stored_key") != null &&
              storageC.localStorageAdaptater.getItem("stored_key") != undefined
            ) {
              // Decrypt the stored key
              const encryptedStoredKey = storageC.localStorageAdaptater.getItem("stored_key");
              const decryptedStoredKey = CryptoJS.AES.decrypt(encryptedStoredKey, sessionSecretKey).toString(CryptoJS.enc.Utf8);
              
              return resolve(decryptedStoredKey);
            }
          }
          ...
          if (
            ...
            const encryptedPrivateKey = CryptoJS.AES.encrypt(decrypted_private_key, sessionSecretKey).toString();
            storageC.localStorageAdaptater.setItem(
              "stored_key",
              encryptedPrivateKey
            );
          ...
          //get auto_logout
          var auto_logout =
            storageC.localStorageAdaptater.getItem("auto_logout");
          if (auto_logout == "true") {
            disconnect();
            //reload page
            location.reload();
            ...
            certhisSecurityResendN.addEventListener("click", async function () {
              if (blockResend == false) {
                ...
                const interval = setInterval(() => {
                  ...
                  certhisSecurityResendN.textContent = `Code Sent. ( Wait ${time_to_show} Secondes for a new code )`;
                  time++;
