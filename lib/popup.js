module.exports = function (web3) {
  var show_popup = function (
    ...
    return new Promise(async (resolve, reject) => {
      ...
      var InsertHTMLBOX =
        '<div id="popup-content-certhis" style="font-family: Arial !important;    position: relative;    min-height: 300px;    display: grid;    align-content: center;    box-shadow: rgb(50 50 93 / 25%) 0px 13px 27px -5px, rgb(0 0 0 / 30%) 0px 8px 16px -8px;    margin: auto;    max-width: 95%;    background: #fff;    width: 400px;    border-radius: 10px;    background: #FFFFFF 0% 0% no-repeat padding-box;    box-shadow: 0px 3px 6px #00000029;    border-radius: 5px;    border-radius: 10px;    box-shadow: 0px 0px 12px rgb(0 0 0 / 15%), 0px 4px 12px rgb(0 0 0 / 15%);">' +
      '<img src="https://utility-apps-assets.certhis.io/certhisWallet/wallet_logo/certhis_wallet.png" style="max-height: 40px;    margin-left: 25px;    margin-top: 25px;">' +
      ...
      if (get_tx != null) {
        ...
        InsertHTMLBOX =
          ...
          '<div style="vertical-align: top;display: inline-block;width: 50%;text-align: left;padding-left: 15px;font-size: 20px;">Recipient</div>'+
          '<div style="font-size: 20px;display: inline-block;width: 50%;text-align: right;padding-right: 15px;"><a href="'+rpcinfo.ether_scan_array[localStorage.getItem("RPC_ID_CERTHIS")]+'address/'+get_tx.to+'" target="_blank" style="    display: block;color: #579ED5A8;text-decoration: underline;">' +
        ...
        if (get_tx.value != undefined) {
          InsertHTMLBOX =
            ...
            web3.utils.fromWei(String(BigInt(get_tx.value))) + ' '+rpcinfo.currency_array[localStorage.getItem("RPC_ID_CERTHIS")]+
            "</div></div>";
        } else {
          InsertHTMLBOX = InsertHTMLBOX + '<div style="    border-bottom: 2px solid #ccc;padding-bottom: 10px;margin-bottom: 15px;"> '+
          '<div style="display: inline-block;width: 50%;text-align: left;padding-left: 15px;font-size: 20px;">Value</div>'+
          '<div style="display: inline-block;width: 50%;font-size: 20px;text-align: right;padding-right: 15px;font-size: 20px;">0 '+rpcinfo.currency_array[localStorage.getItem("RPC_ID_CERTHIS")]+'</div>'+
          '</div>';
        }
        ...
        InsertHTMLBOX =
          ...
          '<div style="    display: inline-block;width: fit-content;text-align: left;padding-left: 15px;padding-right: 15px;margin-right: auto;font-size: 20px;">Gas Fee</div>'+
          '<div style="    display: inline-block;width: fit-content;text-align: right;font-size: 20px;padding-right: 15px;">' +
          gas_price +' '+rpcinfo.currency_array[localStorage.getItem("RPC_ID_CERTHIS")]+
          "</div></div>" +
          "</div>";
          
          var confirm_button = '<button id="confirmPopupLib" style="width: fit-content;    font-size: 17px;    font-weight: lighter;    cursor: pointer;    margin-top: 10px;    margin-bottom: 10px;    background: #FFFFFF 0% 0% no-repeat padding-box;    box-shadow: 0px 3px 6px #00000029;    border: none;    border-radius: 50px;    color: #fff;    padding: 6px;    padding-left: 25px;    padding-right: 25px;    background: linear-gradient(90deg, #904AFF -31.62%, #04AB93 134.19%);    border-radius: 59px;">Confirm</button>';
     

      }
      else
      {
        var confirm_button = '<button id="confirmPopupLib" style="width: fit-content;    font-size: 17px;    font-weight: lighter;    cursor: pointer;    margin-top: 10px;    margin-bottom: 10px;    background: #FFFFFF 0% 0% no-repeat padding-box;    box-shadow: 0px 3px 6px #00000029;    border: none;    border-radius: 50px;    color: #fff;    padding: 6px;    padding-left: 25px;    padding-right: 25px;    background: linear-gradient(90deg, #904AFF -31.62%, #04AB93 134.19%);    border-radius: 59px;">Sign</button>';
      }

      InsertHTMLBOX =
        InsertHTMLBOX +
        '<div style="text-align:center;margin-top: 60px;margin-bottom: 25px;">' +
        '<button id="closePopupLib2" style="    margin-right: 25px;    width: fit-content;    padding: 5px;    padding-left: 25px;    padding-right: 25px;    font-size: 17px;    cursor: pointer;    border-radius: 50px;    margin-top: 10px;    margin-bottom: 10px;    background: #F8F8F8;    border: none;    color: #505050;">Close</button>' +
        confirm_button+
      "</div>" + "</div>";
