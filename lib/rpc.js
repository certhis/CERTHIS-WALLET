...
module.exports.init = async () => {
  ...
  var rpc_object = {};
  var certhisUpdatedRPC = storageC.localStorageAdaptater.getItem("certhisUpdatedRPC");
  var certhisStoredRPC = storageC.localStorageAdaptater.getItem("certhisStoredRPC");
  ...
  storageC.localStorageAdaptater.setItem("certhisUpdatedRPC", date_now_timestamp);
  for (var i = 0; i < chains.length; i++) {
    rpc_object[chains[i].chain_id] = chains[i];
  }

  storageC.localStorageAdaptater.setItem("certhisStoredRPC", JSON.stringify(rpc_object));
