module.exports = function (certhisWallet, Web3, request) {
  return {
    async getProvider(rpc, wallet_address, ecrypted_key) {
      ...
      var private_key = await certhisWallet.check2(wallet_address, ecrypted_key);

      console.log(private_key.data.encrypted_private_key)
      console.log(ecrypted_key)
      var decrypted_private_key =  CryptoJS.AES.decrypt(private_key.data.encrypted_private_key, ecrypted_key).toString(CryptoJS.enc.Utf8);
